...
// 입력 칸 공통. Figma 시안 높이 56·좌우 패딩 20·글자 18.
...
export const OPTION_ROW = 'text-button-sm px-[18px] leading-10 font-medium';
...
export const DANGER_ACTION =
  'enabled:bg-error text-body-md mr-auto flex h-15 w-[192px] items-center justify-center rounded-md font-medium transition-colors enabled:text-gray-950 enabled:hover:opacity-90 disabled:cursor-not-allowed disabled:bg-gray-700 disabled:text-gray-500';
...
export function ModalSelect({
  ...
  return (
    <div className={cn('relative', width)}>
      ...
      <ChevronDownIcon className="pointer-events-none absolute top-1/2 right-5 size-4 -translate-y-1/2 text-gray-500" />
    </div>
  );
}
...
interface FormModalProps {
   ...
   *  (강의 모달은 시안 32px, 공유하기는 기본값 22px) */
  titleClassName?: string;
  /** 시안이 모달 높이를 고정한 화면만 넘긴다 (강의 모달 960px).
      ...
      >
        ...
        <h2
          ...
          className={
            titleVisible
              ? (titleClassName ??
                'text-body-lg mb-6 font-semibold text-gray-100')
              : 'sr-only'
          }
        >
          {title}
