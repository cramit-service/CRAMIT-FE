...
### 4-4. Figma → 코드: 무엇을 1:1로 두고 무엇을 비율로 둘 것인가

Figma 캔버스는 **1920**이지만 실제 브라우저 뷰포트는 그보다 좁다(1920 모니터 + 배율이면 1536).
시안 px을 그대로 박으면 **화면에서 차지하는 비율이 커진다** — 50%로 그린 모달이 1536에서 63%가 된다.
그렇다고 모든 값에 상수를 곱하면(옛 0.72 규칙) 글자까지 줄어 글자/박스 비율이 시안과 어긋난다.
값의 종류에 따라 갈라서 다룬다.

| 무엇                           | 어떻게                                        | 예                                      |
| ------------------------------ | --------------------------------------------- | --------------------------------------- |
| **타이포**                     | 시안 px 그대로. 화면 폭을 따라가지 않는다     | 본문 18px은 어느 화면에서도 18px        |
| **컴포넌트 내부 치수**         | 시안 px 그대로                                | 필드 높이 56, 확정 버튼 346×60, 패딩 20 |
| **뷰포트에 비례하는 컨테이너** | **시안 비율**을 CSS로 적고 시안 px을 상한으로 | 페이지 콘텐츠 열, 모달, 사이드 패널     |

비율은 시안에서 재어 그대로 쓴다. 특정 화면 폭에서 계산한 배율(0.72 같은 상수)을 박지 않는다 —
그러면 그 사람 모니터가 기준이 되어 버린다.

```
콘텐츠 열   1511 / 1830(사이드바 90 제외) = 82.57%  →  lg:w-[82.57%] lg:max-w-[1511px]
모달        960 / 1920 = 50%                      →  w-[min(960px,50vw)]
챗봇 패널   707 / 1920 = 36.82%                    →  min(707px, 36.82vw)
```

- 콘텐츠 열은 바깥 여백을 `px-*`로 주지 말고 **남는 폭이 갖게** 한다(`lg:px-0` + 안쪽에서 `mx-auto`).
  1920에서 좌우 여백이 시안(159/160)과 맞는지로 검산한다.
- 콘텐츠 열 바깥으로 거는 요소(강의 상세의 "이전으로")는 그 여백이 요소보다 좁아지면 잘린다.
  좁은 폭에서는 흐름 안으로 내린다.
- 이 갈래 때문에 컴포넌트는 좁은 화면에서 컨테이너 대비 조금 커 보인다. 글자를 안 줄이는 대가이며,
  그게 맞다고 보고 받아들인 것이다.
...
> **이전 규칙(모든 박스를 0.72배)은 폐기됐다.** 좁은 화면을 보정하려던 방향 자체는 맞았지만,
> 배율이 과했고(1536 기준이면 0.8), 화면마다 다르게 적용됐고, 타이포에는 적용하지 않아
> 글자와 상자의 비율이 시안과 어긋났다.
